...
void handlePuzzleAndInterfaceEvents(SDL_Event &e, GameObjects* gameObjects, Puzzles* puzzles, Textures* textures)
{
   ...
   // Handle the key events for snake puzzle
...
void renderSnakeCipher(Window* window, Textures* textures, GameObjects* gameObjects, Puzzles* puzzles, SDL_Event &e) {
   if (gameObjects->interactionFlag == Interaction::SnakeCipher) {
      ...
      gameObjects->snakeGame.render();

      if (gameObjects->snakeGame.complete[0] == false || gameObjects->snakeGame.complete[1] == false ||
         gameObjects->snakeGame.complete[2] == false || gameObjects->snakeGame.complete[3] == false)
      { 
         gameObjects->snakeGame.update();
      }
      gameObjects->snakeGame.handleEvents(&e);
